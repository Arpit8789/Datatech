...
import { Users, BookOpen, GraduationCap, Clock, TrendingUp, TrendingDown } from 'lucide-react';
...
const AdminDashboard = () => {
  const [stats, setStats] = useState([
    ...
    { name: 'Total Teachers', value: '0', icon: GraduationCap },
    { name: 'Total Courses', value: '0', icon: BookOpen },
    { name: 'Active Users', value: '0', icon: Clock },
  ]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const courseCount = coursesResponse.total;
        ...
        setStats([
          ...
          { 
            ...stats[3], 
            value: Math.floor(studentCount * 0.7).toString() // 70% of students as active users
          },
        ]);

        // For analytics, using calculated values
        setAnalytics({
          newStudents: Math.floor(studentCount * 0.15), // 15% of total as new
          courseCompletion: '78%',
          avgSession: '24 min'
        });
      } catch (error) {
