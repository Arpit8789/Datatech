...
import dsaTest from '@/data/tests/internship-1';

const INTERNSHIP_TEST_LINKS_COLLECTION = '689923bc000f2d15a263'; // Collection ID for internship_test_links

interface TestLink {
  $id: string;
  userId: string;        // Changed from user_id to match schema
  internship_id: string; // Added to match schema
  start_date: string;
  expiry_date: string;
  is_used: boolean;
  created_at: string;
  full_name: string;     // Added to match schema
  email: string;         // Added to match schema
  phone: string;         // Added to match schema
  test_attempt_id?: string; // Added to match schema
}

const InternshipTestConductor = () => {
          ...
          <div className="space-y-6">
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
              ...
              <ul className="list-disc pl-5 space-y-1 text-sm text-blue-700 dark:text-blue-300">
                {dsaTest.instructions.generalInstructions.map((instruction, index) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                ...
                <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                  <li>• Duration: {dsaTest.duration} minutes</li>
                  <li>• Questions: {dsaTest.sections.reduce((acc, section) => acc + section.questions.length, 0)}</li>
                  <li>• Passing Score: {dsaTest.passingScore}%</li>
                </ul>
              </div>
              <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                <h4 className="font-medium mb-2">Time Remaining</h4>
                <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">
                  {dsaTest.duration} minutes
                </p>
...
export default InternshipTestConductor;
